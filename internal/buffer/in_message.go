...
// Constants for buffer/message sizes.
const (
	// MiB is 1 MiB in bytes.
	MiB = 1024 * 1024
)

// MiBPlusPageSize is 1 MiB + hardware page size. Since pageSize is determined
// at runtime, this is a variable rather than a compile-time constant.
var MiBPlusPageSize int

func init() {
	pageSize = unix.Getpagesize()
	MiBPlusPageSize = MiB + pageSize
}
...
var BlockPool1MPlusPage = newBlockPool(8, func() []byte {
	return make([]byte, MiBPlusPageSize)
})
...
func (m *InMessage) AllocBlocks(totalSize int) {
	...
	m.blocks = append(m.blocks, BlockPool1MPlusPage.Get())

	if totalSize > len(m.blocks[0]) {
		remaining := totalSize - len(m.blocks[0])
		num1MBlocks := (remaining + MiB - 1) / MiB
...
func (m *InMessage) FreeBlocks() {
	if len(m.blocks) > 0 {
		BlockPool1MPlusPage.Put(m.blocks[0])
...
func (m *InMessage) ShrinkToFit(n int) {
	...
	for i := usedBlocks; i < len(m.blocks); i++ {
		if i == 0 {
			BlockPool1MPlusPage.Put(m.blocks[i])
		} else {
			BlockPool1M.Put(m.blocks[i])
		}
	}
		...
		if sc, ok := r.(syscall.Conn); ok {
			...
			if err == nil {
				...
				err = rawConn.Control(func(fd uintptr) {
					n, readvErr = unix.Readv(int(fd), m.blocks)
				})
				if err == nil {
					err = readvErr
				}
			...
			return fmt.Errorf("Reader does not support SyscallConn")
		}
	}
